            ...
            if response.status().is_success() {
                match response.json::<serde_json::Value>().await {
                    Ok(data) => {
                        ...
                        let model_available =
                            models.iter().any(|m| m.contains(&config.embedding.model));
...
async fn check_qdrant_connectivity(config: &Config) -> (bool, Vec<String>) {
    info!(
        "Checking Qdrant connectivity at {}",
    ...
    match client.get(&collections_url).send().await {
        Ok(response) => {
            if response.status().is_success() {
                ...
                match QdrantStore::new_with_api_key(
                    &config.storage.qdrant.endpoint,
...
async fn check_sqlite_connectivity(config: &Config) -> bool {
    ...
    match crate::storage::SqliteStore::new(&config.storage.sqlite_path) {
        ...
        Err(e) => {
            error!(
                "✗ Cannot initialize SQLite database at {:?}: {}",
                config.storage.sqlite_path, e
            );
            false
        }
    }
...
fn log_health_status(health: &HealthStatus) {
    ...
    info!(
        "  SQLite: {}",
        if health.sqlite_writable { "✓" } else { "✗" }
    );
...
pub async fn test_embedding_generation(config: &Config) -> Result<()> {
    ...
    if response.status().is_success() {
        ...
        if let Some(embedding) = result.get("embedding").and_then(|e| e.as_array()) {
            info!(
                "✓ Successfully generated test embedding (dimension: {})",
                embedding.len()
            );
            Ok(())
        } else {
            Err(IndexerError::embedding(
                "Embedding response missing 'embedding' field".to_string(),
            ))
        }
    } else {
        Err(IndexerError::embedding(format!(
