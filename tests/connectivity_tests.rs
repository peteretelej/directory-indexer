use directory_indexer::{
    config::{Config, settings::{StorageConfig, QdrantConfig, EmbeddingConfig, IndexingConfig, MonitoringConfig}},
...
async fn test_system_health_check() {
    ...
    if !health.ollama_available {
        ...
        println!("⚠️  Also ensure 'nomic-embed-text' model is installed: ollama pull nomic-embed-text");
    }
...
async fn test_ollama_embedding_generation() {
    ...
    match provider.generate_embeddings(test_texts.clone()).await {
        Ok(response) => {
            println!("✓ Successfully generated {} embeddings", response.embeddings.len());
...
async fn test_qdrant_operations() {
    ...
    let store = match QdrantStore::new_with_api_key(&config.storage.qdrant.endpoint, config.storage.qdrant.collection.clone(), config.storage.qdrant.api_key.clone()).await {
        Ok(store) => {
    ...
    match store.get_collection_info().await {
        Ok(info) => {
            println!("✓ Collection info: {} points, {} indexed vectors", info.points_count, info.indexed_vectors_count);
        }
        Err(e) => {
    ...
    match store.health_check().await {
        Ok(healthy) => {
            println!("✓ Qdrant health check: {}", if healthy { "healthy" } else { "unhealthy" });
...
async fn test_end_to_end_embedding_and_storage() {
    ...
    ).await.expect("Failed to create Qdrant store");
    ...
    let embedding_response = embedding_provider.generate_embeddings(test_texts).await
        .expect("Failed to generate embeddings");
    
    println!("✓ Generated {} embeddings", embedding_response.embeddings.len());
    
    // Create vector points
    use directory_indexer::storage::qdrant::VectorPoint;
    let points: Vec<VectorPoint> = embedding_response.embeddings
        .into_iter()
        .enumerate()
    ...
    // Store in Qdrant
    vector_store.upsert_points(points).await.expect("Failed to upsert points");
    println!("✓ Successfully stored vectors in Qdrant");
    
    // Test search
    let query_embedding = embedding_provider.generate_embedding("machine learning".to_string()).await
        .expect("Failed to generate query embedding");
    
    let search_results = vector_store.search(query_embedding, 5).await
        .expect("Failed to search vectors");
    
    println!("✓ Search returned {} results", search_results.len());
    for (i, result) in search_results.iter().enumerate() {
        println!("  Result {}: {} (score: {:.3})", i, result.file_path, result.score);
    }
    ...
    for i in 0..2 {
        ...
        vector_store.delete_points_by_file(&file_path).await
