...
async fn test_library_search_with_path_scope() {
    ...
    assert!(
        result.is_ok(),
        "Search with path scope should succeed: {:?}",
        result
    );
}
...
async fn test_library_similar_command() {
    ...
    assert!(
        result.is_ok(),
        "Similar files command should succeed: {:?}",
        result
    );
}
...
async fn test_library_get_command() {
    ...
    assert!(
        result.is_ok(),
        "Get full file content should succeed: {:?}",
        result
    );
    ...
    assert!(
        result.is_ok(),
        "Get specific chunk should succeed: {:?}",
        result
    );
    ...
    assert!(
        result.is_ok(),
        "Get chunk range should succeed: {:?}",
        result
    );
}
...
async fn test_library_status_command() {
    ...
    assert!(
        result.is_ok(),
        "Status with text format should succeed: {:?}",
        result
    );
    ...
    assert!(
        result.is_ok(),
        "Status with JSON format should succeed: {:?}",
        result
    );
...
async fn test_library_error_handling() {
    ...
    assert!(
        result.is_err(),
        "Similar with non-existent file should fail"
    );
...
async fn test_library_end_to_end_workflow() {
    ...
    let similar_result =
        similar_internal(readme_path.to_string_lossy().to_string(), 3, false).await;
...
async fn test_library_index_multiple_directories() {
    ...
    fs::write(
        temp_dir1.path().join("file1.md"),
        "Content from directory 1",
    )
    .unwrap();
    fs::write(
        temp_dir2.path().join("file2.md"),
        "Content from directory 2",
    )
    .unwrap();
    ...
    assert!(
        result.is_ok(),
        "Index multiple directories should succeed: {:?}",
        result
    );
}
...
async fn test_library_chunk_operations() {
    ...
    for chunk_spec in invalid_chunks {
        ...
        assert!(
            result.is_err(),
            "Get with invalid chunk '{}' should fail",
            chunk_spec
        );
    }
}

/// Test OpenAI embedding provider when API key is available
#[tokio::test]
async fn test_openai_provider_integration() {
    let api_key = match std::env::var("OPENAI_API_KEY") {
        Ok(key) => key,
        Err(_) => {
            println!("Skipping OpenAI integration test - no API key provided");
            return;
        }
    };

    use directory_indexer::embedding::openai::OpenAIProvider;
    use directory_indexer::embedding::EmbeddingProvider;
    ...
    if !health_result.unwrap() {
        println!("OpenAI health check failed - skipping test");
        return;
    }

    // Test single embedding generation
    let result = provider
        .generate_embedding("Hello, world!".to_string())
        .await;
    assert!(result.is_ok());
    ...
    assert_eq!(batch_response.model, "text-embedding-3-small");
    assert!(batch_response.usage.is_some());
}
